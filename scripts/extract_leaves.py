...
def extract_leaves_with_spans(soup, separate_by_sentence=False):
...
dict_all_spans_by_sentence, dict_all_spans = extract_leaves_with_spans(
    soup, separate_by_sentence=True)
all_leaves_strings = extract_leaves_strings_only(soup)

alignment_filename = 'chat_alignments.txt'


def convert_to_pharaoh(dict_all_spans_by_sentence, alignment_filename):
    # extracting (one or more) digits preceded by underscore, separated by a dash
    pattern = r"_(\d+-\d+)"
    for i, sentence_spans in enumerate(dict_all_spans_by_sentence):
        with open(alignment_filename, 'a+') as file:
            file.write(f'{i}\t')
            for span_id in sentence_spans.keys():
                ...
                if fr_begin == fr_end and eng_begin == eng_end:  # 1 fr to 1 eng, sure alignment
                    file.write(f'{fr_begin}-{eng_begin} ')
                elif fr_begin != fr_end and eng_begin == eng_end:
                    for j in range(int(fr_begin), int(fr_end)+1):  # many fr to 1 eng
                        file.write(f'{j}p{eng_begin} ')
                elif fr_begin == fr_end and eng_begin != eng_end:
                    for j in range(int(eng_begin), int(eng_end)+1):  # 1 fr to many eng
                        file.write(f'{fr_begin}p{j} ')
                elif fr_begin != fr_end and eng_begin != eng_end:  # many fr to many eng
...
convert_to_pharaoh(dict_all_spans_by_sentence, alignment_filename)
